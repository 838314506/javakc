	...
	<body>
						...
						<table class="tree table table-striped table-bordered table-hover table-condensed">
					        ...
					        <tbody>
								<c:forEach var="m" items="${list }" varStatus="v">
									<tr class="treegrid-${m.pe.peid } treegrid-parent-${m.pe.peid }">
										<td>
											<input type="radio" name="peid" value="${m.pe.peid }" value1="${m.pe.pename }" value2="${m.pe.idcard }"/>
										</td>
										<td>${m.pe.pename }</td>
										<td>${m.pe.idcard }</td>
		...
		<script type="text/javascript">
			$(function()
			{
				...
				{
					var count = $('[name=peid]:checked').length;
					if(count == 0)
					{
					...
					{
						var id = $('[name=peid]:checked').val();
						var name = $('[name=peid]:checked').attr('value1');
						var idcard = $('[name=peid]:checked').attr('value2');
						
						parent.$('#peid').val(id);
						parent.$('#pename').val(name);
