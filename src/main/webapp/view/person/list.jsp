	...
	<body>
		<div class="wrapper wrapper-content animated fadeInRight">
			<div class="ibox float-e-margins">
				...
				<div class="col-sm-12">
					<!-- ------------按钮组 start------------ -->
	                <div class="alert alert-success" role="alert">人员详细信息</div>
	                <div class="col-sm-8">
	                	<div class="btn-group hidden-xs" role="group">
	                	
	                	<shiro:hasPermission name="person:create">
			               	  <button type="button" class="btn btn-primary" data-toggle="modal" id="create" name="person/create.jsp">
		                           <i class="glyphicon glyphicon-plus" aria-hidden="true"></i>添加
		                      </button>
	                    </shiro:hasPermission>
	                    
	                    <shiro:hasPermission name="person:update">
		                      <button type="button" class="btn btn-success" data-toggle="modal" id="update" name="person/load.do">
		                          <i class="glyphicon glyphicon-pencil" aria-hidden="true"></i>修改
		                      </button>
	                    </shiro:hasPermission>
	                    
	                    <shiro:hasPermission name="person:delete">
		                      <button type="button" class="btn btn-danger" data-toggle="modal" id="delete" name="person/delete.do">
		                          <i class="glyphicon glyphicon-trash" aria-hidden="true"></i>删除
		                      </button>
	                    </shiro:hasPermission>
	                    
						<shiro:hasPermission name="person:export">
	                        <button type="button" class="btn btn-info" data-toggle="modal" id="export" name="person/export.do">
	                            <i class="glyphicon glyphicon-book" aria-hidden="true"></i>导出
	                        </button>
	                    </shiro:hasPermission>
	                    
	                    <shiro:hasPermission name="person:download">
	                        <div class="btn-group">
							    ...
							    <ul class="dropdown-menu">
							        <li><a href="${path}/personnel/download.do">下载模板</a></li>
	                ...
	                <div class="col-sm-4">
	                	<input class="form-control" id="search" name="pname" value="${entity.pname }" type="" placeholder="查询内容 回车搜索"/>
	                </div>
	                 <!-- ------------按钮组 end------------ -->
		                 ...
		                 <table class="table table-striped table-bordered table-hover table-condensed">
					        ...
					        <tbody>
					        	<c:forEach var="e" items="${page.list }" varStatus="v">
						            <tr>
						                <th><input type="checkbox" name="ids" value="${e.pid }"/></th>
						                <th>${e.pname }</th>
						                <th>${e.idcard }</th>
						                <th><zhg:show value="${e.state }" codeTp="person_state"/></th>
						                <th><zhg:show value="${e.grade }" codeTp="person_grade"/></th>
						                <th><zhg:date times="${e.time }"/></th>
						                <th>${e.heating }</th>
						                <th>${e.eatate }</th>
						                <th><zhg:date times="${e.createDate }"/></th>
						                <th><zhg:date times="${e.updateDate }"/></th>
		...
		<div id="fileUpload" class="wrapper wrapper-content animated fadeInRight" style="display:none;">
			<div class="ibox float-e-margins">
				<form action="${path }/person/download.do" method="post" id="myForm"  enctype="multipart/form-data">
